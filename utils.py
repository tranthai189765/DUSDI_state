...
def update_partition_config(cfg, obs_dim=None, action_dim=None):
    ...
    if cfg.domain in ['dmc_humanoid_state', 'dmc_quadruped_state', 'dmc_hopper_state', 'dmc_cheetah_state']:
...
def make_agent(obs_type, obs_spec, action_spec, num_expl_steps, parent_cfg, cfg):
    ...
    if cfg.name == "dusdi_diayn":
        cfg.domain = parent_cfg.domain
        cfg.env_config = parent_cfg.env

        # load domain-specific parameters
        domain_specific_config = cfg.training_params[cfg.domain]
        for key in domain_specific_config:
            if key in cfg:
                print(f"Overwriting default value of {key} with {cfg[key]}")
            else:
        ...
        with open_dict(cfg):
            cfg.critic_type = "mono"
            if cfg.name != "diayn":
                # Additional DDPG arguments goes here
                cfg.init_temperature = 0.1
                cfg.update_alpha = False
                cfg.sac = False
                cfg.log_std_bounds = [-10, 2]

                cfg.cuda_id = parent_cfg.cuda_id
                cfg.device = None
                cfg.domain = parent_cfg.domain
            if cfg.name == "cic":
                cfg.domain = parent_cfg.domain
