#!/usr/bin/env python3
"""
benchmark_quadruped.py — DUSDi dmc_quadruped_state skill diversity benchmark
...
Usage:
    python benchmark_quadruped.py \\
        ...
        --episode_steps 1000 \\
        --cuda_id      0 \\
        --out_dir      ./benchmark_results_quadruped

Obs layout (78 dims):
    [0:44]  egocentric_state
    [44:78] torso_vel + upright + imu + force_torque
    --> skill_channel=2 partition: [0:44] | [44:78]

Actor input: 78 + 4*2 = 86 dims
"""
...
def parse_args():
    p = argparse.ArgumentParser(description='DUSDi dmc_quadruped_state skill benchmark')
    ...
    p.add_argument('--skill_channel', type=int,   default=2,
                   help='2 channels: egocentric_state | dynamics')
    p.add_argument('--obs_dim',       type=int,   default=78,
                   help='quadruped flat obs: 44+34=78')
    ...
    p.add_argument('--out_dir',       type=str,   default='./benchmark_results_quadruped')
    return p.parse_args()
...
def build_actor(obs_dim, skill_dim, skill_channel, action_dim, hidden_dim, device):
    actor_input_dim = obs_dim + skill_dim * skill_channel  # 78 + 8 = 86
    actor = Actor(
        obs_type='states',
...
def get_xy(env):
    """Quadruped root has a free joint: qpos[0]=x, qpos[1]=y, qpos[2]=z."""
...
def plot_trajectories(skill_xy, all_skills, skill_dim, skill_channel, out_path):
    ...
    ax.set_title(
        f'DUSDi dmc_quadruped_state skill trajectories\n'
...
def main():
    ...
    env = DMCGymEnv('dmc_quadruped_state', max_episode_steps=args.episode_steps)
    ...
    # ── Trajectories CSV ─────────────────────────────────────────────────────
    csv_path = out_dir / 'trajectories_dmc_quadruped_state.csv'
    ...
    print('\n' + '='*55)
    print('DUSDi dmc_quadruped_state Benchmark Summary')
    print('='*55)
